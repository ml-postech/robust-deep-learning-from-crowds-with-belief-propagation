...
def add_train_args(parser):
    ...
    group.add_argument('--lr_confusion', type=float, default=1e-3)
...
def train_epoch(args, epoch, model, train_loader, opt_1, opt_2):
    loss_total = 0.0

    model.train()
    for x, z, ann in train_loader:
        model.zero_grad()
        x, z, ann = x.to(args.device), z.to(args.device), ann.to(args.device)
        loss = model(x, ann)
        loss.backward()
        opt_1.step()
        opt_2.step()
...
def train(args, model, train_loader, test_loader):
    opt_classifier = Adam(model.classifier.parameters(), lr=args.lr)
    opt_workers = Adam([model.confusion], lr=args.lr_confusion)

    for e in range(1, args.n_epochs + 1):
        train_epoch(args, e, model, train_loader, opt_classifier, opt_workers)
