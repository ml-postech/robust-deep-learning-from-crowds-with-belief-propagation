...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.lr_confusion = args.lr_confusion
    ...
    def forward(self, x, ann):
        ...
        for u in range(ann.size(1)):
            ...
            grad_confusion = torch.FloatTensor([[self.given_alpha, self.given_beta], [self.given_beta, self.given_alpha]])
            for t in tasks_of_worker:
                grad_confusion[:, ann[t, u]] += z_inference[t]
            self.confusion[u] = self.confusion[u] - self.lr_confusion * (grad_confusion - self.confusion[u])
